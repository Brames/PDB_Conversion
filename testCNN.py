...
def input1():
  ...
  x1 = Conv2D(64, kernel_size=(3,3), activation='relu')(input1)
  #x1 = Conv2D(32, kernel_size=(3,3), activation='relu')(input1)
  x1 = MaxPooling2D(pool_size=(3,3))(x1)
  x1 = Dropout(drp)(x1)
  x1 = Conv2D(32, kernel_size=(2,2), activation='relu')(x1)
  #x1 = Conv2D(16, kernel_size=(2,2), activation='relu')(x1)
  x1 = Dropout(drp)(x1)
  x1 = Flatten()(x1)
...
x2 = Conv2D(8, kernel_size=(1,3), activation='relu')(input2)
#x2 = Conv2D(8, kernel_size=(1,3), activation='relu')(input2)
x2 = Dropout(drp)(x2)
...
x3 = Conv2D(8, kernel_size=(1,3), activation='relu')(input3)
#x3 = Conv2D(8, kernel_size=(1,3), activation='relu')(input3)
x3 = Dropout(drp)(x3)
#x2 = MaxPooling1D(pool_size=1)(x2)
#x3 = Conv2D(16, kernel_size=(3,3), activation='relu')(x3)
#x3 = Dropout(drp)(x3)
x3 = Flatten()(x3)
...
x4 = concatenate([x2,input4])
x4 = Dropout(drp)(x4)
x4 = Dense(2*X_test.shape[1], activation='relu')(x4)
x4 = Dropout(drp)(x4) 
...
train=[X_image_train_1,X_image_train_2,X_train]
test=[X_image_test_1,X_image_test_2,X_test]
...
for i in range(0,iter):
  model.fit(train, y_train, batch_size=b_size, epochs=ep,verbose=0) #verbose=1 for display bar, 0 for nothing
  ...
  print("Total Epochs: {0} CC: {1} Loss: {2}\n".format(t_ep,CC[0,1],score) )
...
print("\nTest Loss: {0}\n".format(score))
print("Batch Size:{0}\nEpochs {1}\nLearning Rate: {2}\nCC: {3}\n".format(b_size,t_ep,learn,CC[0,1]) )
np.savetxt('/mnt/home/bramerda/Documents/Persistent_Homology/Results/'+sys.argv[1]+'/CCvEpoch_Im23.txt',CC_t)
exit()

#np.savetxt('/mnt/home/bramerda/Documents/Persistent_Homology/Results/'+sys.argv#[1]+'/testCNN_CA_pred_01.txt',finalCC)
...
f = open('/mnt/home/bramerda/Documents/Persistent_Homology/Results/'+sys.argv[1]+'/testCNN_CA_'+sys.argv[2]+'.txt', 'w+')
f.write(("Protein: {0}\n").format(sys.argv[1]))
